...
const SideBar = ({ reminders, setReminders, navItems, colorScheme, isExpanded, setIsExpanded }) => {
  ...
  return (
    <div
      className={`absolute top-0 left-0 h-[100vh] transition-all duration-300 ${isExpanded ? 'w-[15rem]' : ' w-[4rem]'
        }`}
      style={{ backgroundColor: colorScheme.SideBar.background }}
    >
      {isExpanded && (
        <div className="absolute top-[4.5rem] left-[1rem] w-[7mm] bg-transparent cursor-pointer transition-all duration-300"  onClick={handleNewReminder}>
      ...
      <div  className={`max-sm:hidden absolute top-[4.7rem] left-[12rem] w-[8.5mm] bg-transparent cursor-pointer transition-all duration-300 ${isExpanded ? 'left-[12rem] rotate-0' : 'left-[1rem] rotate-180'
          }`} onClick={handleToggle}>
      <svg xmlns="http://www.w3.org/2000/svg"  viewBox="0 0 16 9"><path fill="white" d="M12.5 5h-9c-.28 0-.5-.22-.5-.5s.22-.5.5-.5h9c.28 0 .5.22.5.5s-.22.5-.5.5"/><path fill="white" d="M6 8.5a.47.47 0 0 1-.35-.15l-3.5-3.5c-.2-.2-.2-.51 0-.71L5.65.65c.2-.2.51-.2.71 0s.2.51 0 .71L3.21 4.51l3.15 3.15c.2.2.2.51 0 .71c-.1.1-.23.15-.35.15Z"/></svg>
      </div>
          ...
          <React.Fragment key={index}>
            ...
            <a className="no-underline" href={item.href}>
              ...
              >
                ...
                {isExpanded && (
                  <p
                    className="font-thin text-[14px] pt-[7%] truncate max-w-[150px] ml-[4%]"
